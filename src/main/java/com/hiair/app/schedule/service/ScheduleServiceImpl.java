...
//	private SchedulerFactoryBean schedulerFactoryBean;
	...
	public void scheduleJob(ScheduleJobVO vo) throws SchedulerException, ClassNotFoundException {	
		...
		//org.quartz.ObjectAlreadyExistsException: Unable to store Job : 'IBE.job1', because one already exists with this identification.)
	...
	public void rescheduleJob(TriggerVO orgTrigger) throws SchedulerException, ParseException {
		/*=========================================================
		 * 1. group & trigger명(=> Key) 으로 기존의 Trigger를 get
		 *=========================================================*/
		...
		{	
			...
			/*
			 * 2-1. 새로운 trigger를 define.
			 * ==> 이때, 새로운 trigger의 name, group은 이전과 동일하게 하고, 
			 *     schedule(실행 조건), description(스케쥴 설명) 만 변경.
			 */
	 ...
	 * @throws SchedulerException 
	 */  
	public void addTrigger(Set<TriggerVO> triggers) throws SchedulerException {
	...
	/**
	 * 특정 Job 삭제 - 해당 Job의 trigger들도 모두 삭제
	 * (Delete the identified Job from the Scheduler - and anyassociated Triggers)
	 */
	 ...
	 * QRTZ_TRIGGERS > TRIGGER_STATE : 'PAUSED'
	 */
	...
	/**
	 * 특정 Job 재시작 - 해당 Job의 중지되있던 Trigger들을 재시작
	 * 			       - 중지되어 실행되지않았던 1번 이상의 작업실행?이 발생됨.
	 * (If any of the Job'sTrigger s missed one or more fire-times, then the Trigger's misfireinstruction will be applied.)
	 */
		...
		if("SIMPLE".equals(triggerType)) {
			...
			if(repeatCount > 0) {
				sceduleBuilder = simpleSchedule()
						 ...
						 //.withMisfireHandlingInstructionFireNow() //TODOJ misfire
	...
	public List<TriggerVO> getTriggersOfJob(JobDetailsVO vo) throws SchedulerException {
		...
		for(Trigger qrtzTrigger : scheduler.getTriggersOfJob(vo.getJobKey())){
			triggerList.add(qrtzTriggerToTriggerModel(qrtzTrigger));
		}
		return triggerList;
	}
	...
	public TriggerVO qrtzTriggerToTriggerModel(Trigger qrtzTrigger) throws SchedulerException {
		TriggerVO trigger = new TriggerVO();
		trigger.setJobGroup(qrtzTrigger.getJobKey().getGroup());
		trigger.setJobName(qrtzTrigger.getJobKey().getName());
		trigger.setTriggerGroup(qrtzTrigger.getKey().getGroup());
		trigger.setTriggerName(qrtzTrigger.getKey().getName());
		//TriggerState
		trigger.setDescription(qrtzTrigger.getDescription());
		trigger.setPriority(qrtzTrigger.getPriority());
		trigger.setStartTime(qrtzTrigger.getStartTime());
		trigger.setEndTime(qrtzTrigger.getEndTime());
		trigger.setPrevFireTime(qrtzTrigger.getPreviousFireTime());
		trigger.setNextFireTime(qrtzTrigger.getNextFireTime());
		//org.quartz.Calendar cal = scheduler.getCalendar(qrtzTrigger.getCalendarName());
